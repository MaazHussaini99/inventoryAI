...
function systemEventArb(eventType: string): fc.Arbitrary<SystemEvent> {
  return fc.record({
...
describe('Property 25: Event Bus Delivery', () => {
  ...
  it('every subscriber registered for the event type receives the event', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        async (eventType, numSubscribers) => {
          const localBus = new EventBus();
          await localBus.initialize();

          const received: SystemEvent[][] = Array.from(
            { length: numSubscribers },
            () => []
          );
  ...
  it('no subscriber registered for a different event type receives the event', async () => {
    await fc.assert(
      fc.asyncProperty(
        eventTypeArb,
        eventTypeArb,
        async (publishedType, subscribedType) => {
  ...
  it('mixed subscribers: only matching ones receive, non-matching do not', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        fc.integer({ min: 1, max: 5 }),
        async (targetType, otherType, numTargetSubs, numOtherSubs) => {
          fc.pre(targetType !== otherType);

          const localBus = new EventBus();
          await localBus.initialize();

          const targetReceived: SystemEvent[][] = Array.from(
            { length: numTargetSubs },
            () => []
          );
          const otherReceived: SystemEvent[][] = Array.from(
            { length: numOtherSubs },
            () => []
          );
  ...
  it('published event data is delivered intact to all matching subscribers', async () => {
    await fc.assert(
      fc.asyncProperty(
        eventTypeArb.chain((type) => systemEventArb(type)),
        fc.integer({ min: 1, max: 5 }),
        async (event, numSubscribers) => {
          const localBus = new EventBus();
          await localBus.initialize();
