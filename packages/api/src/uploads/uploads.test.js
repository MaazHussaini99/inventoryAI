...
async function buildAuthenticatedApp(pool) {
...
async function buildUnauthenticatedApp(pool) {
    const app = Fastify({ logger: false });
    await app.register(multipart, {
        limits: { fileSize: 50 * 1024 * 1024 },
    });
...
function createFilePayload(filename, content) {
...
describe('POST /api/uploads', () => {
    ...
    beforeEach(async () => {
        ...
        pool = createMockPool((_query, _params) => {
            return {
                rows: [{
                        id: UPLOAD_ID,
                        status: 'pending',
    ...
    it('should store file with store-scoped path', async () => {
        ...
        expect(mkdir).toHaveBeenCalledWith(expect.stringContaining(STORE_ID), { recursive: true });
        // Verify writeFile was called with the correct path containing uploadId
        expect(writeFile).toHaveBeenCalledWith(expect.stringContaining(`${UPLOAD_ID}-test.csv`), expect.any(Buffer));
    });
    it('should create DataUpload record with correct parameters', async () => {
        ...
        const queryCall = pool._client.query.mock.calls.find((call) => call[0].includes('INSERT INTO data_uploads'));
