...
const quantitySoldArb: fc.Arbitrary<string> = fc
  .integer({ min: 1, max: 10000 })
  .map((n) => n.toString());

/** Generates an optional field value (could be empty or have content) */
const optionalFieldArb: fc.Arbitrary<string> = fc.oneof(
  fc.constant(''),
...
const validRowArb: fc.Arbitrary<Record<string, string>> = fc.record({
...
const invalidRowArb: fc.Arbitrary<Record<string, string>> = fc.oneof(
...
const mixedRowsArb: fc.Arbitrary<{ rows: Record<string, string>[]; expectedValid: boolean[] }> = fc
  .array(
    fc.oneof(
      validRowArb.map((row) => ({ row, valid: true })),
      invalidRowArb.map((row) => ({ row, valid: false }))
    ),
    { minLength: 1, maxLength: 50 }
  )
  .map((items) => ({
 ...
 */
const rawRowWithMappingArb: fc.Arbitrary<{
...
describe('Property 2: Row Validation Partitioning', () => {
  ...
  it('applyMapping + validateRow pipeline produces correct partitioning', () => {
    fc.assert(
      fc.property(
        fc.array(rawRowWithMappingArb, { minLength: 1, maxLength: 30 }),
        (items) => {
          let importedCount = 0;
          let skippedCount = 0;
