...
const standardFieldArb = fc.constantFrom(...STANDARD_FIELDS);
/** Generates a valid source_column string (non-empty, printable) */
const sourceColumnArb = fc.stringOf(fc.char().filter((c) => c.trim().length > 0 && c !== '"' && c !== '\\'), { minLength: 1, maxLength: 50 });
...
const transformArb = fc.oneof(fc.constant(undefined), fc.stringOf(fc.char().filter((c) => c.trim().length > 0), { minLength: 1, maxLength: 30 }));
/** Generates a single valid ColumnMapping */
const columnMappingArb = fc.record({
...
const columnMappingArrayArb = fc
...
const mappableHeadersArb = fc
    .subarray(STANDARD_FIELDS, { minLength: 1, maxLength: 7 })
    .chain((fields) => fc.tuple(...fields.map((field) => fc.constantFrom(...headerKeywordsMap[field]))))
...
describe('Property 1: Column Mapping Round-Trip', () => {
    it('JSON serialization/deserialization of ColumnMapping[] produces identical results', () => {
        fc.assert(fc.property(columnMappingArrayArb, (mappings) => {
    ...
    it('suggestColumnMappings produces deterministic results for the same input headers', () => {
        fc.assert(fc.property(mappableHeadersArb, (headers) => {
    ...
    it('column mapping round-trip preserves all fields including optional transform', () => {
        fc.assert(fc.property(columnMappingArrayArb, (mappings) => {
            ...
            for (const original of withTransforms) {
                const found = deserialized.find((d) => d.source_column === original.source_column && d.target_field === original.target_field);
...
//# sourceMappingURL=column-mapper.property.test.js.map