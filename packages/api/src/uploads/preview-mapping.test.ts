...
function createMockPool(queryFn: (query: string, params?: unknown[]) => unknown) {
  const mockClient = {
    query: vi.fn(queryFn),
    release: vi.fn(),
  };
...
describe('GET /api/uploads/:id/preview', () => {
  ...
  beforeEach(async () => {
    pool = createMockPool((query: string, _params?: unknown[]) => {
  ...
  it('should suggest correct mappings for common headers', async () => {
    ...
    const body = response.json();
    expect(body.suggestedMappings).toBeInstanceOf(Array);
    const targetFields = body.suggestedMappings.map((m: any) => m.target_field);
  ...
  it('should return 404 for non-existent upload', async () => {
    const notFoundPool = createMockPool((query: string) => {
      if (query.includes('SELECT') && query.includes('data_uploads')) {
        return { rows: [] };
      }
      return { rows: [] };
...
describe('POST /api/uploads/:id/mapping', () => {
  ...
  it('should update DataUpload record with mapping and status', async () => {
    ...
    const updateCall = pool._client.query.mock.calls.find(
      (call: unknown[]) => (call[0] as string).includes('UPDATE data_uploads SET column_mapping')
    );

    expect(updateCall).toBeDefined();
    expect(updateCall![1]![0]).toBe(JSON.stringify(mappings));
    expect((updateCall![0] as string)).toContain("'processing'");
  });
  ...
  it('should save ColumnMappingConfig for reuse', async () => {
    ...
    const insertCall = pool._client.query.mock.calls.find(
      (call: unknown[]) => (call[0] as string).includes('INSERT INTO column_mapping_configs')
    );
