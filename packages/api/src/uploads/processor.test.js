...
function createMockClient() {
    ...
    return {
        client,
        pushResult: (result) => queryResults.push(result),
    };
}
...
describe('processUpload', () => {
    ...
    it('should throw if upload is not in processing state', async () => {
        ...
        await expect(processUpload('upload-1', 'store-1', deps)).rejects.toThrow("not in 'processing' state");
    });
    it('should throw if upload has no column mapping', async () => {
        ...
        // UPDATE to failed (catch block)
        mockClient.pushResult({ rows: [], rowCount: 1 });
        await expect(processUpload('upload-1', 'store-1', deps)).rejects.toThrow('no column mapping configured');
    });
    ...
    it('should update existing product when found by name', async () => {
        ...
        const updateProductCall = calls.find((call) => typeof call[0] === 'string' && call[0].includes('UPDATE products'));
        expect(updateProductCall).toBeDefined();
    });
    it('should find existing product by SKU when sku_id is provided', async () => {
        const mappingsWithSku = [
            ...DEFAULT_MAPPINGS,
        ...
        const calls = mockClient.client.query.mock.calls;
        const skuLookupCall = calls.find((call) => typeof call[0] === 'string' && call[0].includes('sku_identifier'));
    ...
    it('should update upload status to completed with correct counts', async () => {
        ...
        const calls = mockClient.client.query.mock.calls;
        const updateUploadCall = calls.find((call) => typeof call[0] === 'string' &&
            call[0].includes('UPDATE data_uploads') &&
            call[0].includes('completed'));
        expect(updateUploadCall).toBeDefined();
        // imported_rows = 1, skipped_rows = 0
        expect(updateUploadCall[1]).toEqual([1, 0, 'upload-1']);
    });
    ...
    it('should handle sale_price with currency symbols', async () => {
        ...
        const calls = mockClient.client.query.mock.calls;
        const salesInsertCall = calls.find((call) => typeof call[0] === 'string' && call[0].includes('INSERT INTO sales_records'));
        expect(salesInsertCall).toBeDefined();
        // The params array should have sale_price = 12.99 at index 5
        const params = salesInsertCall[1];
