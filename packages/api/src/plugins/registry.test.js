...
function createMockPlugin(overrides = {}) {
    return {
        ...
        execute: async (_context) => ({ success: true }),
        shutdown: async () => { },
...
describe('PluginRegistry', () => {
    ...
    describe('validateContract', () => {
        ...
        it('rejects plugin without dependencies array', () => {
            const plugin = createMockPlugin();
            plugin.dependencies = 'not-an-array';
            const result = registry.validateContract(plugin);
            expect(result.valid).toBe(false);
    ...
    describe('activate / deactivate', () => {
        ...
        it('throws if plugin is not registered', async () => {
            await expect(registry.activate('unknown', 'store-1')).rejects.toThrow('Plugin "unknown" is not registered');
        });
        ...
        it('throws if dependency is not active for store', async () => {
            ...
            await registry.register(plugin);
            await expect(registry.activate('test-plugin', 'store-1')).rejects.toThrow('Dependency "dep-plugin" is not active for store "store-1"');
        });
    ...
    describe('execute with fault isolation', () => {
        it('returns success on successful execution', async () => {
            ...
            await registry.register(plugin);
            await registry.activate('test-plugin', 'store-1');
            const context = { storeId: 'store-1', triggeredBy: 'test', correlationId: 'corr-1' };
