...
function createValidPlugin(overrides: Partial<Plugin> = {}): Plugin {
  return {
    id: overrides.id ?? 'test-plugin',
    name: overrides.name ?? 'Test Plugin',
    version: overrides.version ?? '1.0.0',
    dependencies: overrides.dependencies ?? [],
    initialize: overrides.initialize ?? (async (_config: PluginConfig) => {}),
    execute:
      overrides.execute ??
      (async (_context: ExecutionContext): Promise<PluginResult> => ({
        success: true,
      })),
    shutdown: overrides.shutdown ?? (async () => {}),
    healthCheck:
      overrides.healthCheck ??
      (async (): Promise<HealthStatus> => ({ healthy: true, errorCount: 0 })),
  };
}
...
const validPluginObjectArb = fc.record({
  ...
  execute: fc.constant(async (): Promise<PluginResult> => ({ success: true })),
  shutdown: fc.constant(async () => {}),
  healthCheck: fc.constant(async (): Promise<HealthStatus> => ({ healthy: true, errorCount: 0 })),
});
...
describe('Property 23: Plugin Contract Validation', () => {
  ...
  it('accepts objects that implement all required methods and have valid metadata', () => {
    fc.assert(
      fc.property(validPluginObjectArb, (pluginObj) => {
  ...
  it('accepts if and only if all required fields and methods are present and valid', () => {
    fc.assert(
      fc.property(arbitraryObjectArb, (candidate) => {
        ...
        const hasValidVersion =
          typeof obj.version === 'string' && (obj.version as string).trim() !== '';
  ...
  it('rejects objects missing any single required method', () => {
    const requiredMethods = ['initialize', 'execute', 'shutdown', 'healthCheck'] as const;

    fc.assert(
      fc.property(
        fc.constantFrom(...requiredMethods),
        nonEmptyStringArb,
        (methodToRemove, pluginId) => {
  ...
  it('rejects objects with invalid metadata fields', () => {
    ...
    fc.assert(
      fc.property(
        fc.constantFrom('id', 'name', 'version'),
...
describe('Property 24: Plugin Per-Store Isolation', () => {
  ...
  it('activating a plugin for one store does not activate it for another store', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        storeIdArb,
        nonEmptyStringArb,
        async (storeIdA, storeIdB, pluginId) => {
  ...
  it('activation status is independent across arbitrary pairs of stores', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.array(storeIdArb, { minLength: 2, maxLength: 5 }),
        nonEmptyStringArb,
        async (storeIds, pluginId) => {
  ...
  it('multiple failing plugins do not affect a healthy plugin', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        async (storeId, numFailingPlugins) => {
          const registry = new PluginRegistry();
